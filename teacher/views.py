...
def reset_student_progress(request, student_id):
    if request.method == 'POST':
        try:
            ...
            # Delete all exam results for the student
            Result.objects.filter(user=student).delete()
            
            # Reset other related data if needed
...
def signin(request):
    ...
    if request.method == 'POST':
        ...
        if username and password:
            user = authenticate(request, username=username, password=password)  # Ensure username is used if default User model
            if user is not None:
                login(request, user)
                return redirect('teacher_dashboard')
            else:
                messages.error(request, 'Incorrect credentials')  # Use error for failed login
        else:
            messages.error(request, 'Both fields are required.')  # Handle case where fields are empty
